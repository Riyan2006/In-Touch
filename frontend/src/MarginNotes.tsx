...
import { deltaFor, isTextsOnly, rowFor, signalField, type SignalKey } from "./utils";
...
export default function MarginNotes({ contact, month }: { contact: Contact; month: number }) {
  const row = rowFor(contact, month); const textsOnly = isTextsOnly(contact); const available = textsOnly ? signals.filter((signal) => signal.key === "texts") : signals;
  return <><div className={`signal-cards ${textsOnly ? "texts-only-cards" : ""}`}>{available.map((signal) => {
    const value = row[signalField[signal.key]] as number;
    return <motion.article className="signal-card" key={signal.key} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .24 }}><div className="signal-top">{!textsOnly && <span className="signal-icon" aria-hidden>{signal.icon}</span>}<Delta value={deltaFor(contact, signal.key, month)}/></div><p className="signal-value"><Count value={value}/><small>{signal.unit}</small></p><p className="signal-label">{signal.label}</p></motion.article>;
  })}</div>{textsOnly && <p className="texts-only-note">Texts only · calls and meetups are not available from a chat export.</p>}</>;
}
